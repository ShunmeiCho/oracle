...
describe("runOracle GPT-6 Astra reasoning capabilities", () => {
  test.each(["low", "medium", "high", "xhigh", "max"] as const)(
    ...
    async (reasoningEffort) => {
      ...
      expect(client.lastRequest?.tools).toContainEqual(
  ...
  test("forwards Astra standard mode", async () => {
    const client = createClient();
    await runOracle(
      {
        prompt: "Verify Astra standard mode",
        model: "gpt-6-astra",
    ...
    expect(client.lastRequest?.reasoning).toEqual({ effort: "xhigh", mode: "standard" });
  });
